...
_astrometry_param_names = {
    "": {'P', 'e', 'T0', 't0', 'A', 'B', 'F', 'G', 'dx', 'dy', 'dpmra', 'dpmdec'}, # Relative astrometry
    "1": {'P', 'e', 'T0', 't0', 'A1', 'B1', 'F1', 'G1', 'dx', 'dy', 'dpmra', 'dpmdec'}, # Primary astrometry
    "2": {'P', 'e', 'T0', 't0', 'A2', 'B2', 'F2', 'G2', 'dx', 'dy', 'dpmra', 'dpmdec'}, # Secondary astrometry
}
_rv_param_names = {
    "": {'K', 'e', 'T0', 't0', 'P', 'omega', 'systemic_velocity'}, # Relative RVs
    "1": {'K1', 'e', 'T0', 't0', 'P', 'omega1', 'systemic_velocity'}, # Primary RVs
    "2": {'K2', 'e', 'T0', 't0', 'P', 'omega2', 'systemic_velocity'} # Secondary RVs
}
_xyz_param_names = {
    "": {"P", "e", "a", "T0", "t0", "omega", "Omega", "i", 'dx', 'dy', 'dz', 'dpmra', 'dpmdec', 'systemic_velocity'}, # Relative 3D position
    "1": {"P", "e", "a1", "T0", "t0", "omega1", "Omega", "i", 'dx', 'dy', 'dz', 'dpmra', 'dpmdec', 'systemic_velocity'}, # Primary 3D position
    "2": {"P", "e", "a2", "T0", "t0", "omega2", "Omega", "i", 'dx', 'dy', 'dz', 'dpmra', 'dpmdec', 'systemic_velocity'} # Secondary 3D position
}
_vxyz_param_names = {
    "": {"P", "e", "a", "T0", "t0", "omega", "Omega", "i", 'dx', 'dy', 'dz', 'dpmra', 'dpmdec', 'systemic_velocity'}, # Relative 3D velocity
    "1": {"P", "e", "a1", "T0", "t0", "omega1", "Omega", "i", 'dx', 'dy', 'dz', 'dpmra', 'dpmdec', 'systemic_velocity'}, # Primary 3D velocity
    "2": {"P", "e", "a2", "T0", "t0", "omega2", "Omega", "i", 'dx', 'dy', 'dz', 'dpmra', 'dpmdec', 'systemic_velocity'} # Secondary 3D velocity
}
...
class Orbit():
    ...
    def __init__(self, *, velocity_ratio=None, **kwparams):
        ...
        self.derived_params = self.params
    ...
    def _regularize_params(self):
        ...
        for sys in ("", "1", "2"):
            if _astrometry_param_names[sys].issubset(self.covered_params):
                self.astrometry_param_sets[sys] = {}
                for name in _astrometry_param_names[sys]:
                    transform = build_transform_function(self.params, name)
                    self.astrometry_param_sets[sys][name] = transform(**self.params)
                    self.derived_params[name] = self.astrometry_param_sets[sys][name]

            if _rv_param_names[sys].issubset(self.covered_params):
                self.rv_param_sets[sys] = {}
                for name in _rv_param_names[sys]:
                    transform = build_transform_function(self.params, name)
                    self.rv_param_sets[sys][name] = transform(**self.params)
                    self.derived_params[name] = self.rv_param_sets[sys][name]

            if _xyz_param_names[sys].issubset(self.covered_params):
                self.xyz_param_sets[sys] = {}
                for name in _xyz_param_names[sys]:
                    transform = build_transform_function(self.params, name)
                    self.xyz_param_sets[sys][name] = transform(**self.params)
                    self.derived_params[name] = self.xyz_param_sets[sys][name]

            if _vxyz_param_names[sys].issubset(self.covered_params):
                self.vxyz_param_sets[sys] = {}
                for name in _vxyz_param_names[sys]:
                    transform = build_transform_function(self.params, name)
                    self.vxyz_param_sets[sys][name] = transform(**self.params)
                    self.derived_params[name] = self.vxyz_param_sets[sys][name]

        if not (self.astrometry_param_sets[""] or self.astrometry_param_sets["1"] or self.astrometry_param_sets["2"] or
                self.rv_param_sets[""] or self.rv_param_sets["1"] or self.rv_param_sets["2"] or
                self.xyz_param_sets[""] or self.xyz_param_sets["1"] or self.xyz_param_sets["2"] or
                self.vxyz_param_sets[""] or self.vxyz_param_sets["1"] or self.vxyz_param_sets["2"]):
            # Determine which parameters are missing for each type of calculation
    ...
    def astrometry(self, t, system=None):
        ...
        """
        assert system is not None and str(system) in {'1', '2', 'relative'}, f"`system` must be provided for astrometry. It can be either '1', '2', or 'relative'."
        ...
        ref_epoch = self.derived_params['t0']
        if ref_epoch is None:
            ref_epoch = 0.0
        dt = np.asarray(t) - ref_epoch

        M = 2 * np.pi / self.derived_params['P'] * (dt - self.derived_params['T0'])
    ...
    def rv(self, t, system=None):
        ...
        """
        assert system is not None and str(system) in {'1', '2', 'relative'}, f"`system` must be provided for radial velocity. It can be either '1', '2', or 'relative'."
        system = "" if system == "relative" else str(system)
    ...
    def xyz(self, t, system=None):
        ...
        assert system is not None and str(system) in {'1', '2', 'relative'}, f"`system` must be provided for 3D position. It can be either '1', '2', or 'relative'."
        system = "" if system == "relative" else str(system)
    ...
    def vxyz(self, t, system=None):
        ...
        assert system is not None and str(system) in {'1', '2', 'relative'}, f"`system` must be provided for 3D velocity. It can be either '1', '2', or 'relative'."
        system = "" if system == "relative" else str(system)

        if self.vxyz_param_sets[system] is None:
